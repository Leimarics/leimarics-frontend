...
export default function FAQ() {
  ...
  return (
    <section id="faq" className="section bg-brand-bg">
        ...
        <div className="space-y-4">
          {faqs.map((faq, index) => (
            <motion.div
              ...
              className="border border-brand-border rounded-2xl overflow-hidden bg-brand-bg-secondary hover:border-brand-accent/50 transition-colors duration-300"
            >
              <button
                onClick={() => setOpenIndex(openIndex === index ? null : index)}
                className="w-full flex items-center justify-between p-6 text-left focus:outline-none focus:bg-brand-bg transition-colors"
              >
                <span className="font-bold text-brand-text font-display text-lg">
                  {faq.question}
                </span>
                <span className="flex-shrink-0 ml-4 text-brand-accent">
                  {openIndex === index ? (
